			...
			if(strlen($pass) > 15 || strlen($pass) < 8 ){
				echo "
						<script>
							...
							window.location.href='?page=".base64_encode("g_pass_admin")."&user=".base64_encode($_SESSION['user'])."';
						</script>
					 ";
			}else{
				...
				$edit = mysqli_query($conn, "UPDATE t_luser SET pass = '$password' WHERE user = '$kd' " );
