...
$sql = mysqli_query($conn, "SELECT * FROM t_n_rapor WHERE nama = '$kd' ");
		...
		<table class="table table-striped" width="100%">
			...
			<tr>
				...
				<td>
					...
					<a href="?page=<?= base64_encode("nilai_r"); ?>" class="btn btn-warning btn-sm">
	...
	if(isset($_POST['edit'])){
		if($nama == "" || $no_pes == "" || $pai == "" || $ppkn == "" || $ind == "" || $mtk == "" || $sejind == "" || $ing == "" || $senbud == "" || $pjok == "" || $pkwu == "" || $mat_sej == "" || $fis_eko == "" || $kim_sos == "" || $bio_geo == "" || $lm == ""){
			...
			<script type="text/javascript"> 
				...
				window.location.href="?page=<?= base64_encode("nilai_r"); ?>&action=<?= base64_encode("edit_n_r"); ?>&nama=<?= base64_encode($data['nama']); ?>";
			</script>

			<?php
		}else{
				mysqli_query($conn, "UPDATE t_n_rapor SET nama = '$nama', no_pes = '$no_pes', pai = '$pai', ppkn = '$ppkn', ind = '$ind', mtk = '$mtk', sejind = '$sejind', ing = '$ing', senbud = '$senbud', pjok = '$pjok', pkwu = '$pkwu', mat_sej = '$mat_sej', fis_eko = '$fis_eko', kim_sos = '$kim_sos', bio_geo = '$bio_geo', lm = '$lm' WHERE nama = '$kd' ");
				mysqli_query($conn, "INSERT INTO t_history VALUES('', 'edit nilai rapor $nama', '$_SESSION[user]', NULL) ");
				?>
				<script type="text/javascript"> alert("Nilai sudah di Edit"); window.location.href="?page=<?= base64_encode("nilai_r"); ?>"; </script>
